...
## Google Maps API Key
Normally you would keep your API hidden in the backend. However in this project *(and don't be alarmed...)* the Google Maps API Key is public 😮 in `index.html`. The reason for this is that Google actually restricts its usage via the [Cloud Console](https://console.cloud.google.com/). Indeed, the key included here has been restricted. You may only use it on `localhost:3000` and on `*.vercel.app/*`. Feel free to use it for your student project. There may be benefits to getting your own key. If you'd like to get your own key, keep reading!
